...
search2.addEventListener('input', function (e) {
    for (var i = 0; i < list_array2.length; i++) {
        matching2(list_array2[i])
    }
...
search2.addEventListener('keypress', function (e) {
    if (e.keyCode == 13) {
        e.target.value = list_group2.querySelector('[data-highlight="true"]').innerHTML
    }
...
function init_list2() {
    count2 = 0;
    for (var i = 0; i < list_array2.length; i++) {
        init_item2(list_array2[i]);
        list_array2[i].addEventListener('click', copy_paste2);
    }
}

function init_item2(item) {
    item.dataset.display = 'true';
    item.dataset.highlight = 'false';
}

function copy_paste2() {
    search2.value = this.innerHTML;
    // todo : check match of list text and input value for .current 
    init_list2();
    hide_list2(list_group2);
}

function hide_list2(ele) {
    ele.dataset.toggle = 'false'
}

function show_list2(ele) {
...
function key_up_down2() {
    ...
    search2.onkeydown = function (e) {
        ...
        if (e.keyCode === 38) {
            ...
            if (items[count2]) {
                items[count2].dataset.highlight = 'false';
            } else {
        ...
        if (e.keyCode === 40) {
            ...
            if (items[count2 - 1]) {
                items[count2 - 1].dataset.highlight = 'false';
            } else {
...
group2.addEventListener('mouseleave', function (event) {
    if (event.target != list_group2 && event.target.parentNode != list_group2) {
        list_group2.dataset.toggle = 'false'
    }
});